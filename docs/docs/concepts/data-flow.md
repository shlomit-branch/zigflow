...
:::info
Top-level `output.schema` documents and validates the shape of the
workflow output. Use it to describe what the workflow returns. Shape the value
itself with `output.as` on the final task.
:::
